...
func spawn_missile():
	if missile_scene:
		...
		# Визначити точку на лінії, на яку дивиться камера
		var line_length = 20.0  # Визначте відстань, на яку хочете побудувати лінію
		var target_point = camera_position + camera_direction * line_length

		# Можливо, ви хочете передати цю точку снаряду
		missile_instance.target_point = target_point
